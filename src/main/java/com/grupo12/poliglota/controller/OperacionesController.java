...
public class OperacionesController {
    ...
    @PostMapping("/op2/cerrar-sesion") // OP-2: Cierre de sesión y persistencia de progreso (MongoDB + Neo4j + Redis)
    @Operation(
    ...
    public ResponseEntity<?> getDashboardInstructor(
            @Parameter(description = "ObjectId hex del curso, ej: 507f1f77bcf86cd799439011", required = true)
            @RequestParam String cursoId) {
        try {
    ...
    @GetMapping("/op5/recomendar-cursos") // OP-5: Recomendación de próximo curso (Neo4j + MongoDB + Redis)
    @Operation(
