...
public class OP2_CierreSesionService {
    ...
    public CierreSesionResponse cerrarSesion(CierreSesionRequest req) {
        ...
        try {
            ...
            // Actualizar porcentaje en inscripciones (recalculado abajo)
        } catch (RuntimeException e) {
            // Mongo falló → NO borrar HASH; propagar como 500.
        ...
        // ── 4. NEO4J: si completó el curso, marcar (Alumno)-[:COMPLETO]->(Curso) ──
        ...
        if (cursoCompletado) {
            try {
                ...
                log.warn("OP-2: fallo Neo4j al marcar curso completado {}/{}. " +
                         "Mongo y Redis quedaron consistentes, requiere reconciliación.",
                         alumnoId, cursoId, e);
                gradoConsistencia = "parcial";
                avisos.add("No se pudo actualizar el grafo de prerrequisitos en Neo4j. " +
                           "La habilitación de cursos dependientes queda pendiente de reconciliación.");
            }
        }
